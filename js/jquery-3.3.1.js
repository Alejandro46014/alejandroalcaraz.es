...
var i
	...
	// Los espacios en blanco al final y sin escape, capturando algunos caracteres que no son espacios en blanco que preceden al último
...
función createDisabledPseudo (deshabilitado) {
	...
	función de retorno (elem) {
		...
		if ("forma" en elem) {
			...
			// * enumeró los elementos asociados a un formulario en un fieldset deshabilitado
...
Expr = Sizzle.selectors = {
	...
	filtro: {
		...
		"PSEUDO": función (pseudo, argumento) {
			...
			// Priorizar por sensibilidad al caso en caso de que se agreguen pseudos personalizados con letras mayúsculas
	...
	pseudos: {
		...
		// La comparación de C con el valor del idioma del elemento se realiza sin distinción de mayúsculas y minúsculas.
...
tokenize = Sizzle.tokenize = function (selector, parseOnly) {
	...
	while (tanFar) {
		...
		if ((match = rcombinators.exec (soFar))) {
			...
			tokens.push ({
				valor: emparejado,
				// Combinar descendientes combinadores al espacio.
...
función setMatcher (prefiltro, selector, comparador, postfiltro, postFinder, postSelector) {
	...
	return markFunction (función (semilla, resultados, contexto, xml) {
		temp temp, yo, elem,
			...
			matcherIn = preFilter && (selector ||! semilla)?
				...
				if (postFinder) {
					...
					mientras yo-- ) {
						if ((elem = matcherOut [i])) {
							// Restore matcherIn ya que elem aún no es una coincidencia final
...
función matcherFromTokens (tokens) {
	var checkContext, matcher, j,
		...
		// El comparador fundacional garantiza que los elementos sean accesibles desde el contexto (s) de nivel superior
...
function matcherFromGroupMatchers (elementMatchers, setMatchers) {
	var bySet = setMatchers.length> 0,
		...
		superMatcher = function (semilla, contexto, xml, resultados, más externo) {
			...
			// Soporte: IE <9, Safari
			// Tolerar las propiedades de NodeList (IE: "length"; Safari: <number>) haciendo coincidir los elementos por ID
...
// Soporte: IE <9
// Usa getAttributeNode para obtener booleanos cuando se encuentra getAttribute
if (! assert (function (el) {
...
jQuery.fn.extend ({
	...
	es: función (selector) {
		volver !!
			...
			// de modo que $ ("p: first"). is ("p: last") no se devolverá verdadero para un documento con dos "p".
...
jQuery.fn.extend ({
	tiene: función (objetivo) {
		objetivos var = jQuery (objetivo, este),
 ...
 * Posibles opciones:
 *
 * una vez: se asegurará de que la lista de devolución de llamada solo pueda activarse una vez (como un Aplazado)
 *
					...
					Función de resolución (profundidad, diferido, manejador, especial) {
						función de retorno () {
							...
							si (profundidad) {
								...
								// ya que de otro modo se pierde cuando la ejecución se vuelve asíncrona
...
if (document.readyState === "complete" ||
	...
	// Manéjelo de forma asíncrona para permitir que los scripts tengan la oportunidad de demorarse
...
Data.prototype = {
	...
	caché: función (propietario) {
		...
		si (! valor) {
			...
			if (acceptData (propietario)) {
				...
				// Si es un nodo con pocas probabilidades de ser edificado o en bucle
	...
	eliminar: función (propietario, clave) {
		...
		if (clave === undefined || jQuery.isEmptyObject (caché)) {
			...
			// desde los nodos DOM, así que configúralo como indefinido en su lugar
...
función adjustCSS (elem, prop, valueParts, tween) {
	...
	if (initialInUnit && initialInUnit [3]! == unidad) {
		...
		while (maxIterations--) {
			...
			// Evalúe y actualice nuestras mejores conjeturas (duplicando las conjeturas que hacen cero).
...
función showHide (elementos, show) {
	...
	// Configurar la visualización de los elementos en un segundo bucle para evitar un reflujo constante
...
jQuery.event = {
	...
	manejadores: función (evento, manejadores) {
		...
		si (delegateCount &&
			...
			// Soporte: Firefox <= 42
			// Suprimir los clics que violan especificaciones que indican un botón de puntero no primario (trac-3861)
	...
	especial: {
		...
		haga clic en: {
			...
			// Para la casilla de verificación, se desencadena el evento nativo, el estado marcado será correcto
			trigger: function () {
			...
			// Para mantener la coherencia entre navegadores, no active .click () nativo en los enlaces
...
// jQuery.Event se basa en los eventos DOM3 especificados por el enlace de lenguaje ECMAScript
...
jQuery.fn.extend ({
	...
	html: función (valor) {
		acceso de retorno (esta, función (valor) {
			...
			// Vea si podemos tomar un atajo y solo usar innerHTML
...
(función () {
	...
	función computeStyleTests () {
		...
		// Soporte: IE 9 - 11 solamente
		// Detectar informes erróneos de las dimensiones del contenido para el tamaño del cuadro: elementos de cuadro de borde
...
función curCSS (elemento, nombre, computado) {
	...
	si (calculado) {
		...
		// pero el ancho parece ser de forma fiable píxeles.
...
jQuery.extend ({
	...
	css: function (elem, name, extra, styles) {
		...
		// Hacer numérico si es forzado o si se proporcionó un calificador y val parece numérico
		if (extra === "" || extra) {
			num = parseFloat (val);
	...
	if (isBox && elem.nodeType === 1) {
		...
		if (display === "inline" || display === "inline-block" && restoreDisplay! = null) {
			if (jQuery.css (elem, "float") === "none") {
				...
				// Restaure el valor de visualización original al final de las animaciones puras de mostrar / ocultar
				si (! proptween) {
					anim.done (function () {
...
jQuery.fn.extend ({
	...
	acabado: función (tipo) {
		...
		devuelve this.each (function () {
			...
			// Busca animaciones activas y finalízalas.
			para (index = timers.length; index--;) {
...
jQuery.extend ({
	attr: function (elem, name, value) {
		...
		// Agarra el gancho necesario si está definido.
...
jQuery.each ([
	...
	// Eliminar y colapsar espacios en blanco según las especificaciones HTML
...
jQuery.fn.extend ({
	...
	toggleClass: function (value, stateVal) {
		...
		if (typeof stateVal === "boolean" && isValidValue) {
			regresa stateVal? this.addClass (value): this.removeClass (value);
		}
		...
		devuelve this.each (function () {
			...
			// alternar nombre de clase completa
				...
				// De lo contrario, devolver lo que se guardó previamente (si es que hay algo),
	...
	val: función (valor) {
		...
		if (! argument.length) {
			if (elem) {
				...
				// Manejar casos de cadena más comunes
				if (typeof ret === "string") {
					devuelve ret.replace (rreturn, "");
				}
		...
		devuelve this.each (función (i) {
			...
			// Si el conjunto se devuelve sin definir, volver a la configuración normal
...
jQuery.extend (jQuery.event, {
	...
	trigger: function (event, data, elem, onlyHandlers) {
		...
		if (type.indexOf (".")> -1) {
			...
			// disparador de espacio de nombre; crear una expresión regular para que coincida con el tipo de evento en handle ()
		...
		// Determine la ruta de propagación del evento por adelantado, según la especificación de eventos de W3C (# 9951)
		...
		if (! onlyHandlers &&! special.noBubble &&! isWindow (elem)) {
			...
			// Solo agregar ventana si tenemos que documentar (por ejemplo, no obj simple o DOM separado)
...
// que es una violación de especificación - http://www.w3.org/TR/DOM-Level-3-Events/#events-focusevent-event-order
...
jQuery.parseXML = function (data) {
	...
	// Soporte: IE 9 - 11 solamente
	// IE lanza en parseFromString con entrada no válida.
	tratar {
...
jQuery.param = function (a, traditional) {
	...
	// Si se pasó una matriz, asuma que es una matriz de elementos de formulario.
	if (Array.isArray (a) || (a.jquery &&! jQuery.isPlainObject (a))) {
		...
		// lo hizo), de lo contrario codificar params recursivamente.
		para (prefijo en a) {
	...
	serializeArray: function () {
		...
		.map (función (i, elem) {
			...
			if (Array.isArray (val)) {
				devolver jQuery.map (val, function (val) {
...
var
	 ...
	 * 4) se puede usar el símbolo de la trampa "*"
...
Función inspectPrefiltersOrTransports (estructura, opciones, opciones originales, jqXHR) {
	...
	función inspeccionar (tipo de datos) {
		...
		jQuery.each (estructura [dataType] || [], función (_, prefilterOrFactory) {
 ...
 *: encuentra el dataType correcto (media entre el tipo de contenido y el dataType esperado)
 * - Devuelve la respuesta correspondiente.
 * /
función ajaxHandleResponses (s, jqXHR, respuestas) {

	var ct, type, finalDataType, firstDataType,
		contenido = s.contentos,
...
jQuery.extend ({
	...
	// Si se omite el destino, se escribe en ajaxSettings.
	...
	ajax: función (url, opciones) {
		...
		transporte var
			...
			jqXHR = {
				...
				getResponseHeader: function (key) {
					...
					volver partido == nulo? nulo: coincidencia;
				}
		...
		if (s.crossDomain == null) {
			...
			tratar {
				...
				// La propiedad de host del ancla no se establece correctamente cuando s.url es relativo
				...
				// puede ser rechazado por el transporte si es inválido
				s.crossDomain = true;
			}
		}
		...
		if (! s.hasContent) {
			...
			if (s.data && (s.processData || typeof s.data === "string")) {
				...
				// # 9682: elimina datos para que no se utilicen en un reintento final
				eliminar s.data;
			}
			...
			// Poner hash y anti-caché en la URL que será solicitada (gh-1732)
		...
		// Establezca el encabezado Acepta para el servidor, dependiendo del tipo de datos
		...
		// Permitir encabezados / tipos de texto personalizados y aborto temprano
		if (s.foreforeSend &&
		...
		función realizada (status, nativeStatusText, respuestas, encabezados) {
			...
			// Ignorar repetidas invocaciones
			si (completado) {
				regreso;
			}
			...
			// Convertir sin importar qué (de esa manera, los campos de respuestaXXX siempre se configuran)
...
var xhrSuccessStatus = {
		...
		// El protocolo de archivo siempre produce el código de estado 0, asumiendo 200
		0: 200,
	...
	if (support.cors || xhrSupported &&! options.crossDomain) {
		regreso {
			enviar: función (encabezados, completa) {
				...
				// Para las solicitudes de dominios cruzados, considerando que las condiciones para una verificación previa son
...
jQuery.parseHTML = function (data, context, keepScripts) {
	...
	si (! contexto) {
		...
		if (support.createHTMLDocument) {
			...
			// entonces cualquier elemento analizado con URLs
...
jQuery.fn.load = function (url, params, callback) {
	...
	if (self.length> 0) {
		jQuery.ajax ({
			...
			// usuario puede anularlo a través del método ajaxSetup
			tipo: tipo || "OBTENER",
			tipo de datos: "html",
			datos: params
		...
		// Si la solicitud es exitosa, esta función obtiene "datos", "estado", "jqXHR"
...
jQuery.fn.extend ({
	...
	desplazamiento: función (opciones) {
		...
		// Conservar el encadenamiento para setter
	...
	posición: función () {
		...
		// posición: f ixed elementos están desplazados desde la ventana gráfica, que a su vez siempre tiene cero desplazamiento
...
// Crear los métodos de altura interna, anchura interna, altura, anchura, altura externa y anchura exterior
...
jQuery.each (("desenfoque de enfoque de enfoque en enfoque de redimensionamiento, haga clic en clic con el botón" +
	"mousedown mouseup mousemove mouseover mouseover mouseenter mouseleave" +
...
jQuery.proxy = function (fn, context) {
	...
	// esto lanza un TypeError, pero solo devolveremos undefined.
	if (! isFunction (fn)) {
		regreso indefinido;
	}
...
// Regístrese como un módulo AMD nombrado, ya que jQuery se puede concatenar con otros
...
// forma de registrarse. Se usa jquery en minúscula porque los nombres de los módulos AMD son
